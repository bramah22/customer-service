...
def CONTAINER_NAME = "customer-service-" + ENV_NAME
def CONTAINER_TAG = getTag(env.BUILD_NUMBER, env.BRANCH_NAME)
def HTTP_PORT = getHTTPPort(env.BRANCH_NAME)
def EMAIL_RECIPIENTS = "bramah22@gmail.com"
...
node {
    try {
        ...
        stage('Checkout') {
            checkout scm // recuperer toute les branches de git
        }
        ...
        stage('Run App') {
            withCredentials([usernamePassword(credentialsId: 'dockerhubcredrential', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
